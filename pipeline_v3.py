...
class FreeWorldPipelineV3:
    ...
    def _stage7_output(
        ...
        # Generate tracked URLs for INCLUDED jobs only (good/so-so quality jobs)
        # Only jobs with "included" status should get tracking links, not "passed_all_filters" or bad jobs
        included_mask = df.get('route.final_status', '').astype(str).str.startswith('included')
        quality_jobs_df = df[included_mask]  # Process only included jobs (good/so-so quality)
