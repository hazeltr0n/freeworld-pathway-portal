...
def search_memory_jobs(location: str, limit: int = 100, days_back: int = 7, 
    ...
    try:
        ...
        try:
            debug_query = supabase_client.table('pathway_jobs').select('route_type', count='exact').eq('market', market_name).gte('created_at', cutoff_date)
            ...
            local_debug = supabase_client.table('pathway_jobs').select('route_type', count='exact').eq('market', market_name).ilike('route_type', '%local%').gte('created_at', cutoff_date).execute()
