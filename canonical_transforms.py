...
def transform_ingest_outscraper(raw_data: List[Dict], run_id: str, search_location: str = '') -> pd.DataFrame:
    ...
    for _, row in df.iterrows():
        ...
        # Track debug count without excessive printing
        if debug_count < 5:
            debug_count += 1
...
def transform_business_rules(df: pd.DataFrame, filter_settings: Dict[str, bool] = None) -> pd.DataFrame:
    ...
    # Deduplication keys (market-aware for multimarket batches)
    def generate_dedup_keys_with_row_market(row):
        row_market = row.get('meta.market', 'Unknown')
        return generate_dedup_keys(row, row_market)

    print(f"🔍 DEDUP DEBUG: Using per-row market for multimarket deduplication")
    dedup_data = df.apply(generate_dedup_keys_with_row_market, axis=1)
...
def transform_ai_classification(df: pd.DataFrame, ai_results: Dict[str, Dict], job_ids_classified: set = None) -> pd.DataFrame:
    ...
    def apply_ai_result(job_id: str) -> Dict[str, Any]:
        ...
        # Get normalized location from the job row (already processed by LocationNormalizer)
        current_row = df[df['id.job'] == job_id].iloc[0] if not df[df['id.job'] == job_id].empty else None
        normalized_location = current_row.get('source.location_raw', '') if current_row is not None else ''
        ...
        return {
            ...
            'ai.normalized_location': normalized_location,  # Copy from location normalizer output
            'ai.fair_chance': result.get('fair_chance', 'no_requirements_mentioned'),
            'ai.endorsements': result.get('endorsements', 'none_required'),
            'ai.route_type': result.get('route_type', 'Unknown')  # Will be overwritten by route classifier
        }
...
def transform_routing(df: pd.DataFrame, route_filter: str = 'both') -> pd.DataFrame:
    ...
    def determine_final_status(row: pd.Series) -> str:
        """Determine final status based on business rules and route filtering"""

        # Check business rule filters (set by transform_business_rules)
        ...
        # If we get here, job passed all filters including route filter
    ...
    # CRITICAL FIX: Promote quality jobs from 'passed_all_filters' to 'included'
    # This is the missing step that was causing 0 included jobs in analytics
    quality_job_mask = (
        (routing_fields['route.final_status'] == 'passed_all_filters') &
        (df['ai.match'].isin(['good', 'so-so']))
    )

    promotion_count = quality_job_mask.sum() if hasattr(quality_job_mask, 'sum') else 0
    print(f"🎯 ROUTING: Promoting {promotion_count} quality jobs from 'passed_all_filters' to 'included'")

    # Use .loc for proper pandas assignment
    routing_fields['route.final_status'].loc[quality_job_mask] = 'included'

    # Verify the promotion worked
    included_count = (routing_fields['route.final_status'] == 'included').sum()
    print(f"✅ ROUTING: After promotion, {included_count} jobs now have 'included' status")
...
def view_fresh_quality(df: pd.DataFrame) -> pd.DataFrame:
    """Fresh jobs with included status only - for Supabase upload and link generation"""
    if len(df) == 0:
        return df

    # Only include jobs with "included" status (quality jobs that should get tracking links)
    fresh_mask = (
