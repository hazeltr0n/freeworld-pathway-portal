...
class JobMemoryDB:
    ...
    def _check_and_repair_connection(self) -> bool:
        ...
        try:
            ...
            result = self.supabase.table('pathway_jobs').select('job_id').limit(1).execute()
            self._connection_healthy = True
            return True
        except Exception as e:
                ...
                if final_status == 'passed_all_filters' or final_status.startswith('included'):
                    ...
                    if not match or str(match) in ['', 'nan', 'None', 'null']:
                        if final_status.startswith('included'):
                            ...
                            # For passed_all_filters jobs without AI classification, use 'so-so' as placeholder
                            # They will get properly classified later
                            match = 'so-so'
            ...
            for i in range(0, total_records, batch_size):
                ...
                try:
                    ...
                    if result.data is not None:  # RPC returns count or error
                        ...
                        total_stored += count
                        if total_records > batch_size:
                            logger.info(f"✅ Batch {batch_num}/{total_batches}: Stored {count} jobs")
                    else:
                        logger.error(f"❌ Batch {batch_num}/{total_batches}: RPC failed")
                        # Try fallback for this batch
                        result = self.supabase.table('pathway_jobs').upsert(batch).execute()
                        if result.data:
                            total_stored += len(result.data)
                            logger.info(f"✅ Batch {batch_num}/{total_batches}: Stored {len(result.data)} jobs (fallback)")
                        else:
                            logger.error(f"❌ Batch {batch_num}/{total_batches}: Both RPC and upsert failed")

                except Exception as batch_error:
                    logger.warning(f"⚠️ Batch {batch_num}/{total_batches}: RPC failed, trying upsert: {batch_error}")
                    ...
                    try:
                        ...
                        if result.data:
                            ...
                            logger.error(f"❌ Batch {batch_num}/{total_batches}: Upsert failed")
                    except Exception as upsert_error:
                        logger.error(f"❌ Batch {batch_num}/{total_batches}: Both RPC and upsert failed - {upsert_error}")

            if total_stored > 0:
                logger.info(f"✅ Successfully stored {total_stored}/{total_records} job classifications in memory database")
                return True
            else:
                logger.error("❌ Failed to store any job classifications")
                return False
    ...
    def refresh_existing_jobs(self, job_ids: List[str]) -> bool:
        ...
        try:
            ...
            for i in range(0, len(job_ids), batch_size):
                ...
                try:
                    result = self.supabase.table('pathway_jobs').upsert(update_records).execute()
                    if result.data:
    ...
    def check_job_memory(self, job_ids: List[str], hours: int = 168) -> Dict[str, Dict]:
        ...
        try:
            ...
            result = self.supabase.table('pathway_jobs').select('*').in_(
                'job_id', job_ids
    ...
    def search_jobs(self, search_terms: str = None, location: str = None, radius: int = 50, limit: int = 100, hours: int = 72, text_search: bool = False) -> List[Dict]:
        ...
        try:
            ...
            if text_search:
                ...
                query = self.supabase.table('pathway_jobs').select('*').gte('created_at', cutoff_str)
                ...
                query = self.supabase.table('pathway_jobs').select('*').in_(
    ...
    def cleanup_old_records(self, days: int = 7) -> bool:
        ...
        try:
            ...
            # Delete old records
            result = self.supabase.table('pathway_jobs').delete().lt(
    ...
    def get_memory_stats(self) -> Dict:
        ...
        try:
            ...
            result = self.supabase.table('pathway_jobs').select('job_id', count='exact').execute()
            ...
            recent_result = self.supabase.table('pathway_jobs').select('job_id', count='exact').gte(
                'classified_at', cutoff_str
            ).execute()
