...
"""
Career Services Pathway Portal - PRODUCTION ENVIRONMENT
Complete fresh start to bypass Streamlit Cloud import caching
DEPLOYMENT VERSION: September 18, 2025 - Production sync with QA fixes
CACHE_BUSTER_ID: production_sync_v2
"""
...
try:
    ...
    try:
        from PIL import Image
        app_favicon = Image.open("fw_logo.png")
    except (ImportError, FileNotFoundError):
        app_favicon = "🚀"  # Fallback emoji for production

    st.set_page_config(
        page_title="Career Services Pathway Portal",
    ...
    st.markdown("""
    <meta property="og:title" content="Career Services Pathway Portal - CDL Job Search">
    <meta property="og:description" content="Professional CDL job search platform connecting Free Agents with quality driving opportunities through career coach guidance.">
    <meta property="og:type" content="website">
    <meta property="og:site_name" content="Career Services Pathway Portal">
    <meta property="og:image" content="https://fwcareercoach.streamlit.app/fw_logo.png">
    <meta name="twitter:card" content="summary_large_image">
    <meta name="twitter:title" content="Career Services Pathway Portal">
    <meta name="twitter:description" content="Professional CDL job search platform with career coach guidance">
    <meta name="description" content="Career Services Pathway Portal - Professional CDL job search platform connecting Free Agents with quality driving opportunities through personalized career coach guidance.">
...
# Career Coach Authentication
def authenticate_coach():
    """Career Coach login system"""
        ...
        if login_logo:
            ...
            <div class="login-container">
                <div class="login-card">
                    ...
                    <div class="login-subtitle">Career Services Portal</div>
                </div>
            </div>
            """, unsafe_allow_html=True)
        else:
            ...
            if changed_rows:
                ...
                for idx, original, edited in changed_rows:
                    ...
                    updated_agent.update({
                        ...
                        'max_jobs': 1000 if str(edited['Max Jobs']) == "All" else int(edited['Max Jobs']),
...
def show_loan_calculator():
    """Show the loan calculator portal (same app, different URL parameter)"""
    try:
        ...
        st.components.v1.html(calculator_html, height=2000, scrolling=True)
            ...
            for _p in logo_candidates:
                if os.path.exists(_p) and Image:
                    try:
                        ...
                        continue
            # Use FreeWorld logo or fallback emoji
            try:
                app_favicon = Image.open("fw_logo.png") if page_icon_img is None else page_icon_img
            except (FileNotFoundError, OSError):
                app_favicon = "🚀"  # Production rocket emoji
...
def main():
    ...
    # Check for public-facing portals FIRST
    try:
        params = st.query_params
    ...
    # Check for loan calculator portal
    if loan_calculator_param:
        show_loan_calculator()
        st.stop()

    # If not a portal, proceed with coach authentication
    authenticate_coach()
            ...
            st.markdown(f"""
                <div style="display: flex; align-items: center; gap: 20px; margin-top: 0;">
                    ...
                    <h1 style="color: #CDF95C; margin: 0; font-weight: 700;">
                        Career Services Pathway Portal
                    </h1>
                </div>
                """, unsafe_allow_html=True)
        else:
            st.markdown('<h1 style="color: #CDF95C; margin: 0;">🌍 Career Services Pathway Portal</h1>', unsafe_allow_html=True)
            ...
            with st.spinner(spinner_text):
                try:
                    with contextlib.redirect_stdout(indeed_log_buffer), contextlib.redirect_stderr(indeed_log_buffer):
                        # Use same path as memory only for consistent PDF generation
                        df, metadata = pipeline.run_pipeline(params)
    ...
    try:
        ...
        if selected_inner_tab == "📦 Async Batches":
            ...
            with colL:
                ...
                force_fresh_classification = st.checkbox("Force Fresh AI Classification", value=False,
                                                        help="Bypass AI classification cache and re-classify all jobs (more expensive)")
            with colR:
            ...
            if uploaded and run_csv:
                try:
                    ...
                    # Detect file format - Google vs Indeed/Outscraper
                    is_google_format = 'apply_urls' in cols
                    st.info(f"📁 Detected format: {'Google Jobs/Outscraper' if is_google_format else 'Indeed/Generic'}")
                    ...
                    for _, row in df_src.iterrows():
                        ...
                        if is_google_format:
                            # Google format: preserve apply_urls field for proper URL extraction
                            apply_urls = first(row, ["apply_urls"], "")
                            raw = {
                                "title": title,
                                "company": company,
                                ...
                                "salary": first(row, ["salary"], ""),
                                "posted_date": first(row, ["posted_date", "date"], "")
                            }
                        else:
                            # Indeed/Generic format: use viewJobLink
                    ...
                    from canonical_transforms import transform_ingest_outscraper, transform_ingest_google, transform_business_rules
                    from shared_search import MARKET_TO_LOCATION
                    ...
                    # Use correct ingestion function based on detected format
                    if raw_rows:
                        if is_google_format:
                            df_ing = transform_ingest_google(raw_rows, pipe.run_id, search_location)
                        else:
                            df_ing = transform_ingest_outscraper(raw_rows, pipe.run_id, search_location)
                    else:
                        df_ing = ensure_schema(pd.DataFrame())
                    ...
                    df_ai = pipe._stage5_ai_classification(df_dedup, force_fresh_classification=force_fresh_classification)
                    try:
